...
for (const citation of citations) {
  ...
  requires: {
    model: "kev-0.6b",
    why: "At kev-0.6b, two of these citations land below the 0.8 auto-accept floor and go to a person, so the floor has a case to catch here. Each citation is its own request, so the list is worked through one at a time.",
  },
  items: ITEMS,
};
